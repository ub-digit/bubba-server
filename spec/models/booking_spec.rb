...
RSpec.describe Booking, type: :model do
  describe "book pass" do
    before :each do
      ...
      @time_just_past_start = @pass.timestamp_start + 15.minutes
    end
    ...
    it "should automatically set confirmable when close to pass start time" do
      expect(@pass.booked).to be_falsey
      @pass.book("1234567890", "Test Signature", {current_time: @time_close_to_pass, employee: false})
      expect(@pass.booked).to be_truthy
      expect(@pass.booked_by).to eq('1234567890')
      expect(@pass.status).to eq(3)
    end

    it "should automatically confirm a booked pass when past pass start time" do
      expect(@pass.booked).to be_falsey
      @pass.book("1234567890", "Test Signature", {current_time: @time_just_past_start, employee: false})
      expect(@pass.booked).to be_truthy
      expect(@pass.booked_by).to eq('1234567890')
